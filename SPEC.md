...
**Estado:** Spec v0.2 — Fase 0 completa (T0–T6), Fase 1 completa (T7–T9), **Fase 2 en curso: T10–T11 hechos**; siguiente: T12 (§6c) — el lanzamiento (§7) se dispara al completarlo. Enmiendas de implementación en `docs/T0-recon-y-schema.md` §4b–§4d.
**Licencia:** MIT
...
**T11 — Compensation packs** — **hecho** (30-08-2026; formato público con `name`/`description` por pack y `note` humano por entrada, validación estricta con errores campo-exactos — raíz pelada y `$.pre_state` sin `capture` se rechazan al cargar; colisión entre packs = error de arranque, dentro de un pack gana el primer match; `"capture": false` apaga la captura, no el undo: las entradas con capture quedan inertes y las inversas derivadas del resultado siguen activas; proxy sin ledger = packs inertes; pack oficial en `packages/toybox/sagaz-pack.json` con restore por `id`, y regla `delete_tweet → C` en el config del repo; `sagaz packs` lista cobertura y tareas; migración §4d aplicada; reel en `examples/reels/t11-packs.mjs`)
